...
export class NearExpiryComponent implements OnInit  {
  ...
  load(){
    this.client.getBorrowsWithPagination(this.pageNumber,this.pageSize,QueryType.NearExpiry,this.bookId,this.borrowerId,this.created,null,null,null).subscribe(result => {
      this.borrows = result;
    }, error => console.error(error));
  }
  loadSearch() {
